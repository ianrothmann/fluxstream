...
<flux:header container class="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700">
    ...
    <flux:brand href="/" logo="https://fluxui.dev/img/demo/logo.png" :name="config('app.name', 'Fluent')"
                class="max-lg:hidden dark:hidden"/>
    <flux:brand href="/" logo="https://fluxui.dev/img/demo/dark-mode-logo.png" :name="config('app.name', 'Fluent')"
    ...
    @if (auth()->check())
        <flux:dropdown align="end">
            ...
            <flux:menu>
                ...
                <flux:menu.separator/>
                <flux:menu.item icon="power"
                                href="{!! \Illuminate\Support\Facades\URL::signedRoute('logout', ['user' => Auth::user()]) !!}">{{ __('Logout') }}</flux:menu.item>
            </flux:menu>
        </flux:dropdown>
    @else
        <flux:navbar.item class="max-lg:hidden"
                          icon="arrow-right-end-on-rectangle"
                          href="{{route('login')}}"
        >
            {{ __('Log in') }}
        </flux:navbar.item>
        <flux:navbar.item class="max-lg:hidden" icon="key"
                          href="{{route('register')}}">{{ __('Register') }}</flux:navbar.item>
    @endif
</flux:header>
...
<flux:sidebar stashable sticky
    ...
    <flux:navlist variant="outline">
        ...
        @else
            <flux:navlist.item icon="arrow-right-end-on-rectangle"
                               href="{{route('login')}}">
                {{ __('Log in') }}
            </flux:navlist.item>
